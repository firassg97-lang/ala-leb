...
class _MyProfileScreenState extends ConsumerState<MyProfileScreen> {
  ...
  Future<void> _loadPhoneVisibility() async {
    try {
      ...
      // Initialise from profile field (server value takes precedence)
      if (profile != null) {
  ...
  Future<void> _togglePhoneVisibility() async {
    ...
    // Persist to Supabase (best-effort — column may not exist yet)
    try {
      await ref
  ...
  Future<void> _changeAvatar() async {
    ...
    try {
      ...
      final user = ref.read(currentUserProvider)!;
      // Path: {userId}/avatar.jpg (folder per user)
    ...
    return Scaffold(
      body: CustomScrollView(
        ...
        slivers: [
          ...
          // ── Products grid (no ads) ───────────────────────────────────────
          else
            SliverPadding(
              padding: const EdgeInsets.all(8),
              sliver: SliverGrid(
                delegate: SliverChildBuilderDelegate(
                  (_, i) => i < _products.length
                      ? ProductCard(product: _products[i], index: i)
                      : null,
